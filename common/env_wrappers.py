...
def create_procgen_env(config, instance_seed, instance, eval_mode=False):
    ...
    # during evaluation, agent is tested on infinite levels (0), hence create infinite levels
    if eval_mode:
        procgen_args['num_levels'] = 0 # unique level each episode for evaluation
    else:
        procgen_args['num_levels'] = 200 # number of levels to generate for training
...
def create_env_instance(args, instance, decorr_steps, eval_mode=False):
    ...
    elif args.env_name.startswith('procgen:'): env = create_procgen_env(args, instance_seed, instance, eval_mode)
...
def create_env(args, decorr_steps=None, eval_mode=False):
    # setting a high num_parallel_envs for evaluation (like 64 as in train num_parallel_envs) causes broken pipe errors later in training
    parallel_envs = args.eval_parallel_envs if eval_mode is True else args.parallel_envs 
    env_fns = [partial(create_env_instance, args=args, instance=i, decorr_steps=decorr_steps, eval_mode=eval_mode) for i in range(parallel_envs)]
    vec_env = partial(SubprocVecEnvNoFlatten) if args.subproc_vecenv else DummyVecEnvNoFlatten
    env = vec_env(env_fns)
    env = LazyVecFrameStack(env, args.frame_stack, parallel_envs, clone_arrays=not args.subproc_vecenv, lz4_compress=False)
    return env
