...
from common.utils import integrate_q_values
...
class MH_ImpalaCNNLarge(nn.Module):
    """
    Multi Horizon version of IMPALA CNN (large variant)
    """
    def __init__(self, in_depth, actions, linear_layer, num_gammas=1, model_size=1, spectral_norm=False, hyp_vars=None):
        super().__init__()
        def identity(p): return p
        ...
        self.pool = torch.nn.AdaptiveMaxPool2d((8, 8))
        self.hyp_vars = hyp_vars
        
        # create a gamma head for each gamma value
        for i in range(self.num_gammas):
            setattr(
                self, "gamma_head_{}".format(i),
                Dueling(
                    nn.Sequential(
                        linear_layer(2048 * model_size, 256),
                        nn.ReLU(),
                        linear_layer(256, 1),
                    ),
                    nn.Sequential(
    ...
    # options, we can have the agent just learn over multiple horizons, and take max gamma actions, or have the agent also act according to hyp_q value
    def forward(self, x, advantages_only=False):
        Outputs = namedtuple("Outputs", ["hyp_q_vals", "q_vals"])
        q_values = [None] * self.num_gammas
        
        f = self.main(x)
        f = self.pool(f)
        
        for i in range(self.num_gammas):
            q_values[i] = getattr(self, 'gamma_head_{}'.format(i))(f, advantages_only=advantages_only)
        
        hyperbolic_q_values = integrate_q_values(q_values, self.hyp_vars.integral_estimate, self.hyp_vars.eval_gammas, len(self.hyp_vars.eval_gammas), self.hyp_vars.gammas)
        
        output = Outputs(hyperbolic_q_values, q_values)
        return output


def get_model(model_str, spectral_norm, hyp_vars=None):
    if model_str == 'nature': return NatureCNN
        ...
        print('Creating impala_large')
        return partial(ImpalaCNNLarge, model_size=int(model_str[13:]), spectral_norm=spectral_norm)
    elif model_str.startswith('mh_impala_large:'): # get the model_size (one of 1, 2 and 4) after string splitting
        print('Creating mh_impala_large')
        return partial(MH_ImpalaCNNLarge, model_size=int(model_str[16:]), spectral_norm=spectral_norm, hyp_vars=hyp_vars)